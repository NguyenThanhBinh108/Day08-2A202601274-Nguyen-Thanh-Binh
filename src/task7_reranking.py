...
def rerank_cross_encoder(
    ...
    if not jina_api_key:
        raise RuntimeError("JINA_API_KEY chưa được set trong .env — cần key để dùng Jina Reranker API")
...
def rerank(
    ...
    if method == "cross_encoder":
        ...
        if not candidates or "embedding" not in candidates[0]:
            raise ValueError("rerank_mmr cần candidates có key 'embedding'")
        ...
        # candidates có thể là 1 list phẳng (rescoring theo rank của chính nó) hoặc đã
        # là list-of-lists từ nhiều ranker (gọi trực tiếp rerank_rrf trong trường hợp đó).
